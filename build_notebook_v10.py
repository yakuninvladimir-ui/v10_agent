...
def generate_notebook() -> pathlib.Path:
    ...
    cells = [
        make_markdown_cell([
            ...
            f"**Configuration**: Concurrency=6, Reasoning=xhigh, Context=64K..131K, MTP=3 (Graceful Fallback), HeavySmoke={ENABLE_PHASE_A_HEAVY_SMOKE}.\n",
        ], "a1b2c3d0"),
        make_code_cell([
            "# =============================================================================\n",
        ...
        make_code_cell([
            ...
            "            'LCLD_MAX_ACTIONS_PER_LEVEL': '80',\n",
            "            'LCLD_GAME_WALL_CLOCK_LIMIT_SECONDS': '5000',\n",
            "            'LCLD_GAME_CONCURRENCY': '6',\n",
            ...
            "            'ARC_VLLM_ENABLE_PREFIX_CACHING': '0',\n",
            "            'ARC_VLLM_ENABLE_CHUNKED_PREFILL': '1',\n",
            "            'ARC_VLLM_ASYNC_SCHEDULING': '1',\n",
            "            'ARC_VLLM_NO_ENABLE_LOG_REQUESTS': '1',\n",
            "            'ARC_VLLM_DISABLE_UVICORN_ACCESS_LOG': '1',\n",
            "        }\n",
            ...
            "        try:\n",
            "            import serving_teardown\n",
            "            serving_teardown.teardown_serving(port=phase_a_heavy_smoke.VLLM_PORT, working_dir=working_root)\n",
            "        except Exception:\n",
            "            pass\n",
            "        try:\n",
            ...
            "\n",
            "        # 7. Start vLLM Server Watchdog (15s probe, 2 auto-restarts, process-identity teardown)\n",
            "        import vllm_server_watchdog as watchdog\n",
            "        import serving_teardown\n",
            "        import serving_setup\n",
            "        watchdog_setup = serving_setup.ServingSetup(\n",
            "            host=phase_a_heavy_smoke.VLLM_HOST,\n",
            "            port=phase_a_heavy_smoke.VLLM_PORT,\n",
            "            working_dir=working_root,\n",
            "            start_fn=lambda: phase_a_heavy_smoke.start_vllm_server(model_path, site_packages),\n",
            "            stop_fn=phase_a_heavy_smoke.stop_vllm_server,\n",
            "        )\n",
            ...
            "        results = run_concurrent_arcade_games(arcade, concurrency=6)\n",
            ...
            "            print(f'[Phase B] Watchdog stop error: {stop_exc}', flush=True)\n",
            "        try:\n",
            "            import serving_teardown\n",
            "            serving_teardown.teardown_serving(\n",
            "                port=getattr(phase_a_heavy_smoke, 'VLLM_PORT', 1234),\n",
            "                working_dir=working_root,\n",
            "            )\n",
            "        except Exception as td_exc:\n",
            "            print(f'[Phase B] Serving teardown error: {td_exc}', flush=True)\n",
            "        try:\n",
            "            phase_a_heavy_smoke.stop_vllm_server()\n",
            "        except Exception:\n",
            "            pass\n",
